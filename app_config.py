...
JSON_FORMAT_SPECS = {
    ...
    "2D Bounding Boxes": "Detect the relevant objects in this image and provide their 2D bounding boxes and labels. The answer should follow the json format: [{\"box_2d\": [ymin, xmin, ymax, xmax], \"label\": \"description\"}, ...]. Coordinates are normalized to 0-1000. IMPORTANT: The coordinates should be in a flat array [ymin, xmin, ymax, xmax], not as separate objects with properties. Return ONLY valid JSON without any additional text or markdown formatting.",
    "Segmentation Masks": "Give the segmentation masks for the objects in this image. Output a JSON list of segmentation masks where each entry contains the 2D bounding box in the key \"box_2d\" (as [ymin, xmin, ymax, xmax] normalized to 0-1000), the segmentation mask in key \"mask\" (as base64 encoded PNG image data), and the text label in the key \"label\". Use descriptive labels. Limit to 10 items. Return ONLY valid JSON without any additional text or markdown formatting.",
